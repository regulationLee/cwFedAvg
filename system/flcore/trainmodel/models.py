...
class BaseHeadSplit_list(nn.Module):
    def __init__(self, args, cid):
        super().__init__()

        self.base = eval(args.model[cid % len(args.model)])

        if hasattr(self.base, 'heads'):
            head = copy.deepcopy(self.base.heads)
            self.base.heads = nn.Identity()
        elif hasattr(self.base, 'head'):
            head = copy.deepcopy(self.base.head)
            self.base.head = nn.Identity()
        elif hasattr(self.base, 'fc'):
            if self.base.__class__.__name__ == 'ShuffleNetV2':
                head = nn.Linear(in_features=self.base.fc.in_features, out_features=args.num_classes)
            else:
                head = copy.deepcopy(self.base.fc)
            self.base.fc = nn.Identity()
        elif hasattr(self.base, 'classifier'):
            if self.base.__class__.__name__ == 'EfficientNet':
                head = nn.Linear(in_features=self.base.classifier[1].in_features, out_features=args.num_classes)
            else:
                head = copy.deepcopy(self.base.classifier)
...
class BaseHeadSplit_prototype(nn.Module):
    def __init__(self, args):
        super(BaseHeadSplit_prototype, self).__init__()
